...
# 1. Sample T1 at 25%, 50%, 75% of the cortical thickness, at the grey-white matter boundary, and 0.5mm and 1mm subcortically
# 2. Smooth the following features using a 10mm gaussian kernel:
# - cortical thickness
# - T1 at 25%, 50%, 75% of the cortical thickness, at the grey-white matter boundary, and 0.5mm and 1mm subcortically
...
do
  ...
  for h in $H
  do
#if T1 exists, then sample it
    if [ -e "$s"/mri/brain.mgz ]
     then
    # Sample T1 at 25%, 50%, 75% of the cortical thickness & at the grey-white matter boundary & smooth using 10mm Gaussian kernel
    D="0.5 0.25 0.75 0"
      for d in $D
      do
        #sampling volume to surface
        mri_vol2surf --src "$s"/mri/brain.mgz --out "$s"/surf_meld/"$h".gm_T1_"$d".mgh --hemi "$h" --projfrac "$d" --srcreg "$s"/mri/transforms/Identity.dat --trgsubject "$s" --surf white
        #smoothing
        mris_fwhm --s "$s" --hemi "$h" --cortex --smooth-only --fwhm 10\
        --i "$s"/surf_meld/"$h".gm_T1_"$d".mgh --o "$s"/surf_meld/"$h".gm_T1_"$d".sm10.mgh
      done

    # Sample T1 0.5mm and 1mm subcortically & smooth using 10mm Gaussian kernel
    D_wm="0.5 1"
    for d_wm in $D_wm
    do
      mri_vol2surf --src "$s"/mri/brain.mgz --out "$s"/surf_meld/"$h".wm_T1_"$d_wm".mgh --hemi "$h" --projdist -"$d_wm" --srcreg "$s"/mri/transforms/Identity.dat --trgsubject "$s" --surf white
      mris_fwhm --s "$s" --hemi "$h" --cortex --smooth-only --fwhm 10\
      --i "$s"/surf_meld/"$h".wm_T1_"$d_wm".mgh --o "$s"/surf_meld/"$h".wm_T1_"$d_wm".sm10.mgh
    done
    fi
